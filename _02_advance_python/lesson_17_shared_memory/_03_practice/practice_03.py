...
def compute_partial_sum(sh_array, start, end, sh_sum, lock):
    ...
    # print(f'Частичная сумма ({start}-{end}): {local_sum}')  # используются индексы значений
...
def main():
    ...
    for i in range(num_processes):
        ...
        process = Process(target=compute_partial_sum, args=(shared_array, start, end, shared_sum, lock))
