...
while True:
    try:
        days_count = int(input('Введите количество дней для анализа: '))
        if days_count < 0:
            raise OnlyPositiveException
        else:
            break
    except OnlyPositiveException:
        print(f'Ошибка ввода: значение не может быть отрицательным')
    except ValueError:
...
if days_count > 0:
    ...
    print(f'В ввели 0 дней для анализа, расчет невозможен.')
