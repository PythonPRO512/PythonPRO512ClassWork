...
only_event2 = event2_guests_set.difference(event1_guests_set)

# Вывод результатов
print(f'Посетили оба мероприятия: {', '.join(both_events)}')
print(f'Были только на первом: {', '.join(only_event1)}')
print(f'Были только на втором: {', '.join(only_event2)}')